...
class Reconstructor:
    ...
    def get_surface_info(
        ...
        """
        # Calculate the gradients，求真实表面梯度
        ...
        # Calculate the height map，高度计算，poisson_dct_neumaan(G[:, :, 0], G[:, :, 1])用泊松方程对梯度场进行积分，恢复高度图。
        #第0/1通道分别是x/y方向的梯度
        H = poisson_dct_neumaan(G[:, :, 0], G[:, :, 1]).astype(np.float32)

        # Calculate the contact mask，接触掩码计算
        if self.contact_mode == "standard":
            # Filter by color difference
            diff_image = image.astype(np.float32) - self.bg_image.astype(np.float32)#计算每个像素与背景的颜色差。
            color_mask = np.linalg.norm(diff_image, axis=-1) > color_dist_threshold#颜色差大于阈值的区域认为是接触区
